...
const position=[51.505, -0.09]
        ...
        ReactDOM.render(
                                  <h1>10 Lowest Median Electricity usage by post code</h1>
                                  ,document.getElementById('head1')
                        );


let jsonstr= async() => {
                var x= await axios({url: process.env.REACT_APP_APIADD + '/api/medianeleclow'});
                  return x.data;
...
const TEST = jsonstr().then((data)=> {
	const vals = [];
    	...
    	for (let i = 0; i < data.length; i++) {
        	vals.push('{"name": "' + data[i].pcode + '", "median": ' + data[i].median + '}');
